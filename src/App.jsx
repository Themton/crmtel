...
function InlineStatusDropdown({ value, statuses, onChange }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button onClick={() => setOpen(!open)} style={{ padding: "2px 8px", borderRadius: 6, border: "none", background: cur?.color || "#9ca3af", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4 }}>{cur?.label || "—"} <I.ChevDown /></button>
...
function SubjectDropdown({ value, subjects, onChange }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button onClick={() => setOpen(!open)} style={{ padding: "2px 8px", borderRadius: 6, border: "none", background: cur?.color || "#9ca3af", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4 }}>{value || "เลือก"} <I.ChevDown /></button>
...
function OfferDropdown({ value, onChange }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button onClick={() => setOpen(!open)} style={{ padding: "4px 12px", borderRadius: 8, border: "none", background: cur?.color || "#9ca3af", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4 }}>{value || "เลือก"} <I.ChevDown /></button>
      {open && <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, background: "#fff", borderRadius: 12, boxShadow: "0 8px 30px rgba(0,0,0,0.18)", border: "1px solid #e5e7eb", padding: 6, zIndex: 200, minWidth: 140, animation: "fadeIn .15s" }}>
        ...
        {OFFER_OPTIONS.map((o) => (
          <button key={o.value} onClick={() => { onChange(o.value); setOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 10px", border: "none", background: value === o.value ? "#fffbeb" : "transparent", borderRadius: 8, cursor: "pointer", textAlign: "left" }}
            onMouseEnter={(e) => { if (value !== o.value) e.currentTarget.style.background = "#f8fafc"; }} onMouseLeave={(e) => { if (value !== o.value) e.currentTarget.style.background = "transparent"; }}>
            <span style={{ padding: "3px 10px", borderRadius: 6, fontSize: 14, fontWeight: 700, color: "#fff", background: o.color }}>{o.value}</span>
            {value === o.value && <span style={{ marginLeft: "auto", color: "#d4a017" }}><I.Check /></span>}
          </button>
        ))}
      </div>}
...
function EditableCell({ value, onSave, type = "text", style: sx = {} }) {
  ...
  return (
    <div onClick={() => setEditing(true)} style={{ cursor: "pointer", padding: "2px 4px", borderRadius: 4, border: "1px solid transparent", minHeight: 18, ...sx }}
      ...
      <span style={{ fontSize: 15, color: value ? "#1e293b" : "#c0c0c0", display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{display}</span>
    </div>
  );
}
...
function InlineAssignDropdown({ value, employees, onSave }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <div onClick={() => setOpen(!open)} style={{ cursor: "pointer", padding: "2px 4px", borderRadius: 4, border: "1px solid transparent", fontSize: 15, fontWeight: 600, color: value ? "#92400e" : "#d97706", whiteSpace: "nowrap" }}
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#fde68a"; e.currentTarget.style.background = "#fffbeb"; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = "transparent"; e.currentTarget.style.background = "transparent"; }}>
        {value || "ยังไม่มอบหมาย"}
      </div>
      {open && <div style={{ position: "absolute", top: "100%", left: 0, background: "#fff", borderRadius: 10, boxShadow: "0 6px 20px rgba(0,0,0,0.18)", border: "1px solid #e5e7eb", width: 200, zIndex: 200, animation: "fadeIn .1s" }}>
        <div onClick={() => { onSave(""); setOpen(false); }} style={{ padding: "6px 12px", cursor: "pointer", fontSize: 14, color: "#9ca3af", borderBottom: "1px solid #f3f4f6" }}
        ...
        <div style={{ maxHeight: 200, overflowY: "auto" }}>
          {employees.map((em, i) => (
            <div key={em.id} onClick={() => { onSave(em.name); setOpen(false); }}
              ...
              <div style={{ width: 22, height: 22, borderRadius: "50%", background: colors[i % colors.length], display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "#fff" }}>{(em.nickname || em.name).slice(0, 1)}</div>
              <span style={{ fontSize: 14, fontWeight: value === em.name ? 700 : 400 }}>{em.nickname || em.name}</span>
              {value === em.name && <span style={{ marginLeft: "auto", color: "#3b82f6", fontSize: 14 }}>✓</span>}
            </div>
          ))}
        </div>
      </div>}
...
function CRMApp({ currentUser, onLogout }) {
  ...
  const COL_DEFS = {
    ...
    received_product: { label: "ได้รับสินค้า", render: (c) => c.received_product ? <span style={{ color: "#059669", fontWeight: 600, cursor: "pointer", fontSize: 14 }} onClick={() => upd(c.id, "received_product", false)}>✓ได้รับ</span> : <span style={{ color: "#d97706", cursor: "pointer", fontSize: 14 }} onClick={() => upd(c.id, "received_product", true)}>รอส่ง</span> },
    ...
    nickname: { label: "ชื่อเล่น", render: (c) => <span style={{ fontSize: 15, color: "#6b7280", whiteSpace: "nowrap" }}>{(() => { const emp = employees.find((e) => e.name === c.assigned_to); return emp?.nickname || ""; })()}</span> },
    created_at: { label: "วันที่สร้าง", render: (c) => <span style={{ fontSize: 14, color: "#9ca3af", whiteSpace: "nowrap" }}>{c.created_at ? (() => { try { return new Date(c.created_at).toLocaleDateString("th-TH", { day: "2-digit", month: "short", year: "numeric" }); } catch { return c.created_at; } })() : "—"}</span> },
  };
  ...
  return (
    <div style={{ fontFamily: "'Sarabun','Noto Sans Thai',sans-serif", background: "#f0f2f5", minHeight: "100vh", color: "#1a1a2e" }}>
      ...
      <header style={{ background: "linear-gradient(135deg, #3d2a0a, #2a1d06)", padding: "0 32px", height: 64, display: "flex", alignItems: "center", gap: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.15)" }}>
        ...
        {USE_DEMO && <span style={{ background: "#fbbf24", color: "#78350f", fontSize: 14, padding: "2px 10px", borderRadius: 12, fontWeight: 600 }}>DEMO</span>}
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setShowNotif(!showNotif)} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer", padding: 6, position: "relative", fontSize: 20 }} className={notifications.length > 0 ? "bell-shake" : ""}>
              🔔
              {notifications.length > 0 && <span style={{ position: "absolute", top: 0, right: 0, background: "#ef4444", color: "#fff", fontSize: 15, fontWeight: 700, borderRadius: "50%", width: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>{notifications.length}</span>}
            </button>
                ...
                {notifications.length === 0 ? (
                  ...
                  <div key={n.id} style={{ padding: "12px 16px", borderBottom: "1px solid #f3f4f6", background: "#fffbeb" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: 14, color: "#9ca3af" }}>จาก {n.from_user} • {(() => { try { return new Date(n.created_at).toLocaleString("th-TH", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }); } catch { return ""; } })()}</span>
                      <button onClick={async () => { await supabase.from("crm_notifications").update({ read: true }).eq("id", n.id); setNotifications(notifications.filter((x) => x.id !== n.id)); }} style={{ background: "none", border: "none", color: "#d4a017", fontSize: 14, cursor: "pointer", fontWeight: 600 }}>✓ อ่านแล้ว</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <main style={{ flex: 1, padding: 28, overflowY: tab === "customers" ? "hidden" : "auto", overflowX: "hidden" }}>
          ...
          {tab === "dashboard" && (() => {
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 14, color: "#22c55e", fontWeight: 600 }}>⚡ เรียลทาม</span>
                  ...
                  {stats.filter((_, i) => i > 0).map((s, i) => {
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 15, color: "#6b7280", textAlign: "center", fontWeight: 600, lineHeight: 1.2 }}>{s.l}</span>
                      </div>
                    );
                  })}
                </div>
                    ...
                    {dailyKeys.reverse().map((d) => {
                      ...
                      return (
                        <div key={d} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                          ...
                          <span style={{ fontSize: 14, color: "#6b7280", textAlign: "center", fontWeight: 600, lineHeight: 1.1 }}>{dateStr}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <tbody>
                    {[...dailyKeys].reverse().map((d, idx) => {
                      ...
                      return <tr key={d} style={{ borderBottom: "1px solid #f3f4f6", background: idx === 0 ? "#fffbeb" : "transparent" }}>
                        <td style={{ padding: "10px 14px", fontWeight: idx === 0 ? 700 : 400, color: idx === 0 ? "#1e40af" : "#374151" }}>{dateStr} {idx === 0 && <span style={{ background: "#d4a017", color: "#fff", padding: "2px 8px", borderRadius: 10, fontSize: 15, fontWeight: 700, marginLeft: 6 }}>ล่าสุด</span>}</td>
                        ...
                        <td style={{ padding: "10px 14px", fontSize: 14 }}>{Object.entries(dailyCalls[d].byEmp || {}).sort((a, b) => b[1] - a[1]).map(([name, cnt]) => <span key={name} style={{ display: "inline-block", padding: "2px 8px", borderRadius: 6, background: "#fef3c7", color: "#92400e", fontWeight: 600, fontSize: 14, marginRight: 4, marginBottom: 2 }}>{name} ({cnt})</span>)}</td>
                      </tr>;
                    })}
                  </tbody>
                </table>
          ...
          {tab === "customers" && <div>
              ...
              <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #e5e7eb", overflowX: "auto" }}>
                {[
                  ...
                  <button key={t.key} onClick={() => setToolbarTab(toolbarTab === t.key ? null : t.key)}
                    style={{ padding: "10px 20px", border: "none", background: toolbarTab === t.key ? "#d4a017" : (t.key === "employee" && empFilter.length > 0 ? "#fef3c7" : "transparent"), color: toolbarTab === t.key ? "#fff" : (t.key === "employee" && empFilter.length > 0 ? "#92400e" : "#6b7280"), fontWeight: 600, fontSize: 15, cursor: "pointer", whiteSpace: "nowrap", borderRadius: toolbarTab === t.key ? "8px 8px 0 0" : 8, display: "flex", alignItems: "center", gap: 6 }}>
                    {t.icon} {t.label}
                  </button>
                ))}
                <div style={{ flex: 1 }} />
                ...
                <div style={{ position: "relative", zIndex: 100 }}>
                  <div style={{ position: "absolute", top: 4, left: 20, background: "#fff", borderRadius: 14, boxShadow: "0 8px 30px rgba(0,0,0,0.18)", border: "1px solid #e5e7eb", width: 560, zIndex: 100, animation: "fadeIn .15s" }}>
                    ...
                    <div style={{ padding: "12px 16px" }}>
                          ...
                          {(() => {
                            ...
                            return opts.length > 0 ? (
                              <div style={{ position: "relative", flex: 1 }}>
                                <div onClick={(e) => { e.currentTarget.nextSibling.style.display = e.currentTarget.nextSibling.style.display === "none" ? "block" : "none"; }} style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid #e5e7eb", fontSize: 14, cursor: "pointer", background: "#fff", minHeight: 30, display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
                                  {selected.length === 0 && <span style={{ color: "#9ca3af" }}>เลือก ({opts.length})</span>}
                                  {selected.slice(0, 3).map((sv) => { const o = opts.find((x) => x.v === sv); return <span key={sv} style={{ padding: "1px 8px", borderRadius: 4, background: "#fef3c7", color: "#92400e", fontSize: 14, fontWeight: 700 }}>{o?.l || sv}</span>; })}
                                  {selected.length > 3 && <span style={{ fontSize: 15, color: "#9ca3af" }}>+{selected.length - 3}</span>}
                                </div>
                                <div style={{ display: "none", position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.15)", border: "1px solid #e5e7eb", zIndex: 200, maxHeight: 200, overflowY: "auto", marginTop: 4 }}>
                                  {opts.map((o) => (
                                    <label key={o.v} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", cursor: "pointer", fontSize: 14, background: selected.includes(o.v) ? "#fffbeb" : "transparent" }}
                      ...
                      <button onClick={() => setAdvFilters([...advFilters, { field: "", op: "contains", value: "" }])} style={{ background: "none", border: "none", color: "#374151", fontSize: 15, cursor: "pointer", padding: "8px 0", display: "flex", alignItems: "center", gap: 4 }}>+ เพิ่มตัวกรอง</button>
                    </div>
                  ...
                  <div style={{ position: "absolute", top: 4, left: 20, background: "#fff", borderRadius: 14, boxShadow: "0 8px 30px rgba(0,0,0,0.18)", border: "1px solid #e5e7eb", width: 320, zIndex: 100, animation: "fadeIn .15s" }}>
                    ...
                    <div style={{ maxHeight: 320, overflowY: "auto", padding: "4px 0" }}>
                      <div onClick={() => { setEmpFilter([]); setToolbarTab(null); }}
                        ...
                        <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#d4a017", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "#fff" }}>✦</div>
                        <span style={{ fontSize: 15, color: "#374151", fontWeight: empFilter.length === 0 ? 700 : 400 }}>ทั้งหมด</span>
                      ...
                      <div onClick={() => { setEmpFilter(["__none__"]); setToolbarTab(null); }}
                        ...
                        <span style={{ fontSize: 15, color: "#374151", fontWeight: empFilter.includes("__none__") ? 700 : 400 }}>ยังไม่มอบหมาย</span>
            ...
            {selectedRows.length > 0 && <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 20px", background: "#fffbeb", borderBottom: "1px solid #fde68a" }}>
              <span style={{ fontSize: 14, color: "#92400e", fontWeight: 600 }}>✓ เลือกแล้ว {selectedRows.length} รายการ {selectedRows.length > pagedFc.length ? "(ข้ามหน้า)" : ""}</span>
              <div style={{ display: "flex", gap: 8 }}>
                {selectedRows.length < fc.length && <button onClick={() => setSelectedRows(fc.map((c) => c.id))} style={{ background: "none", border: "none", color: "#d4a017", fontSize: 14, cursor: "pointer", fontWeight: 600 }}>เลือกทั้งหมด {fc.length}</button>}
                <button onClick={() => setSelectedRows([])} style={{ background: "none", border: "none", color: "#6b7280", fontSize: 14, cursor: "pointer" }}>ยกเลิก</button>
              </div>
            </div>}
            <div style={{ background: "#fff", borderRadius: "0 0 14px 14px", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
              <div className="crm-scroll" style={{ overflowX: "scroll", overflowY: "auto", height: "calc(100vh - 260px)" }}>
                <table style={{ borderCollapse: "collapse", fontSize: 15, tableLayout: "fixed", width: "max-content", minWidth: "100%" }} className="crm-table">
                  <thead style={{ position: "sticky", top: 0, zIndex: 5 }}><tr style={{ background: "#f8fafc", borderBottom: "2px solid #e5e7eb" }}>
                    <th style={{ padding: "4px 6px", width: 30, fontSize: 15, fontWeight: 700, color: "#9ca3af" }}>#</th>
                    <th style={{ padding: "4px 6px", width: 55 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                        ...
                        <select onChange={(e) => { if (e.target.value === "page") setSelectedRows(pagedFc.map((c) => c.id)); else if (e.target.value === "all") setSelectedRows(fc.map((c) => c.id)); else setSelectedRows([]); e.target.value = ""; }} style={{ border: "1px solid #e5e7eb", borderRadius: 4, background: "#fff", color: "#d4a017", fontSize: 15, cursor: "pointer", padding: "1px 2px", fontWeight: 700 }}>
                          <option value="">▾</option>
                  ...
                  <tbody>
                    {pagedFc.map((c, idx) => (
                      <tr key={c.id} style={{ borderBottom: "1px solid #f0f0f0", height: 22 }} onMouseEnter={(e2) => (e2.currentTarget.style.background = "#fafafa")} onMouseLeave={(e2) => (e2.currentTarget.style.background = "transparent")}>
                        <td style={{ padding: "1px 6px", color: "#b0b0b0", fontSize: 14, textAlign: "center" }}>{(safePage - 1) * PAGE_SIZE + idx + 1}</td>
                        ...
                        <div onMouseEnter={(e) => { if (window._colStatsTimer) clearTimeout(window._colStatsTimer); e.currentTarget.style.background = "#fef3c7"; const rect = e.currentTarget.getBoundingClientRect(); setColStats((prev) => ({ x: Math.min(rect.left, window.innerWidth - 240), y: 0, key, label: COL_DEFS[key]?.label || key, showMenu: prev?.key === key ? prev.showMenu : false })); }}
                          ...
                          {mode === "histogram" ? (
                            <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                              <span style={{ fontSize: 14, color: "#9ca3af", whiteSpace: "nowrap" }}>นับ ▼</span>
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                              <span style={{ fontSize: 14, color: "#9ca3af", whiteSpace: "nowrap" }}>{statLabels[mode] || "นับ"} ▼</span>
                              <span style={{ fontSize: 14, fontWeight: 700, color: "#3b82f6" }}>{val}</span>
                            </div>
                          )}
                        </div>
                      </td>;
          ...
          {tab === "supervisor" && (() => {
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <button onClick={async () => { const res = await supabase.from("crm_activity_log").select().order("created_at", { ascending: false }).range(0, 199); if (res.data) setActivityLog(res.data); showToast("โหลดประวัติแล้ว"); }} style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", fontSize: 14, cursor: "pointer", color: "#6b7280" }}>🔄 รีเฟรช</button>
              </div>
                  ...
                  <tbody>
                    ...
                    {activityLog.map((log) => {
                      ...
                      return <tr key={log.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                        <td style={{ padding: "8px 14px", color: "#9ca3af", whiteSpace: "nowrap", fontSize: 14 }}>{timeStr}</td>
                        <td style={{ padding: "8px 14px", fontWeight: 600 }}>{log.user_name}</td>
                        <td style={{ padding: "8px 14px" }}><span style={{ padding: "2px 10px", borderRadius: 6, fontWeight: 600, fontSize: 14, color: "#fff", background: color }}>{log.action}</span></td>
          ...
          {tab === "employees" && <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <span style={{ fontSize: 14, color: "#9ca3af" }}>จัดการบัญชีที่ระบบ crm-themt2</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
              {employees.map((e2) => {
                ...
                return <div key={e2.id} style={{ background: "#fff", borderRadius: 14, padding: 24, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", position: "relative", borderLeft: custCount > 0 ? "4px solid #d4a017" : "4px solid #e5e7eb" }}>
                  ...
                  <span style={{ padding: "4px 12px", borderRadius: 20, fontSize: 14, fontWeight: 600, background: custCount > 0 ? "#fef3c7" : "#f3f4f6", color: custCount > 0 ? "#92400e" : "#9ca3af" }}>{custCount} ลูกค้า</span>
                </div>;
              })}
            </div>
          </div>}
                  ...
                  <div>
                    {f === "assigned_to" && (() => {
                      ...
                      return <div>
                        ...
                        {/* Equal mode summary */}
                        {sel.length > 1 && assignMode === "equal" && <div style={{ background: "#fffbeb", borderRadius: 6, padding: "6px 10px", fontSize: 14, color: "#92400e" }}>กระจายเท่ากัน {selectedRows.length} ลูกค้า → {sel.length} คน</div>}
                          ...
                          <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden" }}>
                            ...
                            {promoKeys.map((pk) => {
                              ...
                              return <div key={pk} style={{ display: "grid", gridTemplateColumns: "140px 30px 1fr", padding: "10px 12px", alignItems: "start", borderBottom: "1px solid #f3f4f6" }}>
                                <span style={{ fontSize: 15, paddingTop: 6 }}><span style={{ padding: "2px 8px", borderRadius: 6, background: "#fef3c7", color: "#92400e", fontWeight: 700, fontSize: 14 }}>{pk}</span> <span style={{ color: "#9ca3af", fontSize: 14 }}>({promoGroups[pk]} คน)</span></span>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                  ...
                                  {pkSel.length > 1 && <div style={{ fontSize: 15, color: "#ea580c", padding: "2px 8px" }}>กระจาย {promoGroups[pk]} คน → {pkSel.length} คน ({Math.floor(promoGroups[pk] / pkSel.length)}-{Math.ceil(promoGroups[pk] / pkSel.length)} คน/คน)</div>}
                                </div>
                              </div>;
                            })}
                          </div>
                          {Object.values(promoMap).some((v) => v?.length > 0) && <div style={{ background: "#fff7ed", borderRadius: 6, padding: "8px 10px", marginTop: 8, fontSize: 14, color: "#92400e" }}>
                            สรุป: {sel.map((name) => {
                              let count = 0;
          ...
          <div style={{ padding: 24 }}>
            ...
            {multiAddEmp.map((emp, idx) => (
              <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 100px 120px 100px 120px 100px 40px", gap: 8, marginBottom: 8, alignItems: "center" }}>
                <input value={emp.name} onChange={(e) => { const n = [...multiAddEmp]; n[idx].name = e.target.value; setMultiAddEmp(n); }} placeholder="ชื่อพนักงาน" style={{ padding: "8px 10px", borderRadius: 8, border: "1px solid #d1d5db", fontSize: 15, outline: "none" }} />
          ...
          <div style={{ padding: 24 }}>
            ...
            {importResult.dupes.length > 0 && <div style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                }} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #d97706", background: "#fffbeb", color: "#d97706", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>⬇ ดาวน์โหลดเบอร์ซ้ำ</button>
              </div>
        ...
        <div style={{ position: "fixed", left: Math.max(10, Math.min(colStats.x, window.innerWidth - (colStats.showMenu ? 420 : 240))), bottom: 40, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.15)", border: "1px solid #e5e7eb", display: "flex", zIndex: 3000, animation: "fadeIn .1s", overflow: "hidden", maxHeight: "60vh" }}
          ...
          {(() => {
            ...
            return <div style={{ width: 220, maxHeight: 380, overflowY: "auto", borderRight: "1px solid #f3f4f6" }}>
              <div style={{ padding: "10px 14px", fontSize: 14, fontWeight: 600, color: "#6b7280", borderBottom: "1px solid #f3f4f6" }}>อัตราส่วน</div>
              {sorted.map(([label, cnt], i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 14px", fontSize: 15 }}>
                  ...
                  <span style={{ fontSize: 14, color: "#9ca3af", whiteSpace: "nowrap" }}>({cnt}/{total})</span>
                </div>
              ))}
            </div>;
          })()}
          ...
          {colStats.showMenu && <div style={{ width: 160, borderLeft: "1px solid #f3f4f6" }}>
            {[
              ...
              <div key={item.key} onClick={() => {
                ...
                style={{ padding: "8px 14px", cursor: "pointer", fontSize: 15, color: item.highlight ? "#3b82f6" : "#374151", background: footerStats[colStats.key] === item.key ? "#eff6ff" : "transparent", fontWeight: footerStats[colStats.key] === item.key ? 700 : 400 }}
